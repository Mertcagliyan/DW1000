...
uint8_t broadcastAddress[] = {0xC8, 0xF0, 0x9E, 0x53, 0x13, 0x0C};
...
void loop()
{
  if(number == 2){
    ...
    }else if(number3 == 5 && number4 == 7){
      if (isAnchor) {
            // Start as anchor
            //DW1000Ranging.startAsAnchor(ANCHOR_ADD, DW1000.MODE_LONGDATA_RANGE_LOWPOWER, false);
            strcpy(outgoingReadings.msg, "5");
            esp_err_t result = esp_now_send(broadcastAddress, (uint8_t *) &outgoingReadings, sizeof(outgoingReadings));
            number3 = 0;
            number4 = 0;
        } else {
